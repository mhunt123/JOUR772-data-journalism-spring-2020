...
ggplot(maryland_pills_per_year) +
 geom_bar(stat="identity", aes(year, dosage_unit), fill="orange") +
  labs(x="Year", y="Total pills", title="In Maryland, opioids use rose until 2011 and then steadily declined into 2014", subtitle = "Total pills shipped to Maryland by year", caption = "Source: DEA ARCOS database, via Washington Post") +
...
#Baltimore spiked in 2006, while Maryland did not.
...
#PART 1
by_state_total_pills <- arcos_county_pills_per_year %>%
  filter(year=="2012") %>%
...
```{r}
#PART3
state_population_2012 <- by_state_total_pills%>%
