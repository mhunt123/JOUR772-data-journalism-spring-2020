...
mingo %>%
  group_by(transaction_year,buyer_county) %>%
summarise(total_pills = n(),
            total_pills = sum(dosage_unit) 
            ) %>%
  arrange((transaction_year))

#there was an increase in pills between 2008-2009 but then a steady decrease after those years. It declined by half between 2006-2012
