...
```{r}
#Load Mingo County data from data folder
...
```{r}
#Get the full summary of the data
summary(mingo)
```
## Light Cleaning
...
#Uniting Distrubutor Data Together 
mingo_working <- mingo %>%
...
#Added the function median to summarise function
mingo %>%
  group_by(buyer_name) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit), 
...
#The most significant combo: Actavis Pharma, Inc, MIAMI-LUKEN, and STROSNIDER.	
...
#I would create a pivot table grouping Cardinal Health and then subsidizing the reporter state or zip code. 
```
## Submission
