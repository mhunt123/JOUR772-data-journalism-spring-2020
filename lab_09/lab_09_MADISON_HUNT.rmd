...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Average Pills Per Person", y="Median Household income", title="", caption = "Source: DEA ARCOS database, via Washington Post")
...
correlation_pillsincome <- pills_population_income %>%
  ungroup() %>%
  select(median_household_income, average_pills_per_person)
