...
summary(baltimore)
#REPORTER_ZIP, BUYER ZIP,TRANSACTION_ID is stored as a dbl that doesn't need to be counted
#29,908 rows 
#42 columns
#null is represented as missing values in this data set
...
#RITE AID PHARMACY HAD THE MOST PILLS SENT TO IT
```
...
#MCKESSON CORPORATION IS RESPONSIBLE FOR SENDING THE MOST PILLS TO BALTIMORE CITY, the sent 58267330 pills, and they are located in Maryland
```
...
```{r}
# Professional Pharmacy Service and its going to Sinai Hospital 
```
...
#Rite Aid of Maryland, it was responsible for 100% of the pils, and there were 16740590 pils sent to the whole city
...
baltimore %>%
  group_by(buyer_name) %>%
   summarise(shipments = n(),
    ...
    mutate(shipments_rank = percent_rank(shipments)) %>%
    arrange(desc(shipments_rank)) %>%
    #filter(buyer_name == "RITE AID OF MARYLAND, INC.")
